...
public class MenuBar extends JMenuBar
{
    ...
    public void addBlockingSettingsAction(Action action)
    {
        JMenuItem blocking = new JMenuItem(action);
        blocking.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F4, 0));
        blocking.setMnemonic('B');

        settingsMenu.add(blocking);
    }
    
    public void addForwardingSettingsAction(Action action)
    {
        JMenuItem forwarding = new JMenuItem(action);
        forwarding.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F9, 0));
        forwarding.setMnemonic('F');

        settingsMenu.add(forwarding);
    }
    
    public void addBillingSettingsAction(Action action)
    {
        JMenuItem billing = new JMenuItem(action);
        billing.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F6, 0));
        billing.setMnemonic('P');

        settingsMenu.add(billing);
    }
