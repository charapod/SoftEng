...
/******************************************************************************/
     ...
     */
	public synchronized void processRegister(Request request,
			SipProvider sipProvider, ServerTransaction serverTransaction) {
		try {
			Header user = request.getHeader(Constants.USERNAME);
			Header pass = request.getHeader(Constants.PASSWORD);
			...
			try {
				...
				ProxyDebug.println("Could not get the username or password from the login request");
				return;
			}
			System.out.println("Username = " + username+ ", Password = "+ password);
			if (!registerServer.areCredentialsValid(username, password)) {
				System.out.println("INVALID");
				registerServer.sendBadCredentials(request, sipProvider, serverTransaction);
				return;
			}
			...
			if (ProxyDebug.debug) {
				ProxyDebug.println("Registrar, processRegister(), key: \""
						+ key + "\"");
			}
			if (key == null) {
				if (ProxyDebug.debug) {
					ProxyDebug
							.println("Registrar, processRegister(), key is null"
			 ...
			 * header field. If not, it skips to the last step. If the Contact
			 * header field is present, the registrar checks if there is one
			 * Contact field value that contains the special value "*" and an
			 * Expires field. If the request has additional Contact fields or an
			 * expiration time other than zero, the request is invalid, and the
			 * server MUST return a 400 (Invalid Request) and skip the remaining
			 * steps. If not, the registrar checks whether the Call-ID agrees
			 * with the value stored for each binding. If not, it MUST remove
			 * the binding. If it does agree, it MUST remove the binding only if
			 * the CSeq in the request is higher than the value stored for that
			 * binding. Otherwise, the update MUST be aborted and the request
			 * fails.
			 */
			...
			if (!hasContactHeaders(request)) {
				...
				if (contactHeaders != null) {
					for (int i = 0; i < contactHeaders.size(); i++) {
						ContactHeader contact = (ContactHeader) contactHeaders
								.elementAt(i);
				...
				if (ProxyDebug.debug) {
					ProxyDebug
							.println("Registrar, processRegister(), response sent:"
									+ response.toString());
				}
				return;
			}
			...
			if (hasStar(request)) {
				...
				if (contactHeaders.size() > 1) {
					if (ProxyDebug.debug) {
						ProxyDebug
								.println("Registrar, processRegister(), more than one contact header"
				...
				if (!hasExpiresZero(request)) {
					if (ProxyDebug.debug) {
						ProxyDebug
								.println("Registrar, processRegister(), expires time different from"
										+ " 0 with a wild card."
										+ " 400 INVALID REQUEST replied");
					}
					Response response = messageFactory.createResponse(
				...
				if (ProxyDebug.debug) {
					ProxyDebug
							.println("Registrar, processRegister(), (* and expires=0) "
			...
			if (registrationsTable.hasRegistration(key)) {
				...
				if (proxy.getConfiguration().rfc2543Compatible
					...
					System.out.println("CISCO IP PHONE FIX:  "
							+ "Updating proper registration for " + key);
			...
			if (ProxyDebug.debug) {
				ProxyDebug
						.println("Registrar, processRegister(), internal error, "
								+ "exception raised:");
				ProxyDebug.logException(ex);
			}
		}
