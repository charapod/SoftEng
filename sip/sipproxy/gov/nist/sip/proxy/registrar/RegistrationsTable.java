...
public class RegistrationsTable {
	...
	public synchronized boolean hasRegistration(String key) {
		...
		if (res)
			ProxyDebug
					.println("RegistrationsTable, hasRegistration(), Checking registration for \""
	...
	protected void addRegistration(String key, Request request)
		...
		ProxyDebug
				.println("RegistrationsTable, addRegistration(), registration "
	...
	public void removeContact(String key, ContactHeader contactHeader) {
		...
		if (registration != null) {
			...
			if (!registration.hasContacts()) {
				ProxyDebug
						.println("RegistrationsTable, removeContact(), the registration: "
								+ key
	...
	public void updateRegistration(String key, Request request)
			throws Exception {
		ProxyDebug
				.println("RegistrationsTable, updateRegistration(), registration updated"
						+ " for the key: " + key);
	...
	public Vector getContactHeaders(String key) {
		...
		if (registration == null)
			...
			return registration.getContactsList();
	}

	public void startTimer(String key, int expiresTime,
			ContactHeader contactHeader) {
		// we kill the precedent timer related to this key if there is one:
		Address address = contactHeader.getAddress();
		javax.sip.address.URI cleanedUri = Registrar.getCleanUri(address
				.getURI());
		String contactURI = cleanedUri.toString();

		// ifdef SIMULATION
		/*
		 * SimTimer oldTimer; //else
		 */
		Timer oldTimer;
		// endif
		//
		...
		// else
		/*
		 * synchronized(expiresTaskTable) { oldTimer =
		 * (SimTimer)expiresTaskTable.get(contactURI); } //endif
		 */
		if (oldTimer != null) {
			ProxyDebug
					.println("RegistrationsTable, startTimer(), An old timer has "
	...
	protected void printRegistrations() {
		...
		for (Enumeration e = registrations.keys(); e.hasMoreElements();) {
			...
			Registration registration = (Registration) registrations
					.get(keyTable);
