...
import gov.nist.sip.proxy.softeng.BillingServer;
import gov.nist.sip.proxy.softeng.BlockingServer;
import gov.nist.sip.proxy.softeng.Constants;
import gov.nist.sip.proxy.softeng.DBServer;
import gov.nist.sip.proxy.softeng.ForwardingServer;
import gov.nist.sip.proxy.softeng.RegisterServer;
 ...
 *
 * @author Olivier Deruelle <deruelle@nist.gov> M. Ranganathan <mranga@nist.gov>
 *         (convert to simulation) <br/>
...
public class Proxy implements SipListener {
	...
	protected RegisterServer registerServer;
	...
	public Proxy(String confFile) throws Exception {
		...
		if (confFile == null) {
			...
			try {
				...
				ProxyConfigurationHandler handler = new ProxyConfigurationHandler(
						confFile);
				configuration = handler.getConfiguration();
				if (configuration == null
						|| !configuration.isValidConfiguration()) {
					System.out
							.println("ERROR: the configuration file is not correct!"
					...
					blockingServer = BlockingServer.getInstance();
					blockingServer.init(dbServer, this);
					forwardingServer = ForwardingServer.getInstance();
					forwardingServer.init(dbServer, this);
					billingServer = BillingServer.getInstance();
					billingServer.init(dbServer, this);
					registerServer = RegisterServer.getInstance();
					registerServer.init(dbServer, this);
					registrar.setRegisterServer(registerServer);
				}
			} catch (Exception ex) {
				System.out
						.println("ERROR: exception raised while initializing the proxy");
				ex.printStackTrace();
				throw new Exception(
	...
	public void processRequest(RequestEvent requestEvent) {
		...
		try {
			...
			if (!requestValidation.validateRequest(sipProvider, request,
					serverTransaction)) {
				...
				if (ProxyDebug.debug)
					ProxyDebug
							.println("Proxy, processRequest(), the request has not been"
			...
			if (serverTransaction == null) {
				...
				if (method.equals(Request.INVITE)
					...
					try {
						...
						TransactionsMapping transactionsMapping = (TransactionsMapping) serverTransaction
						...
						if (ProxyDebug.debug)
							ProxyDebug
									.println("Proxy, processRequest(), this request"
			...
			if (requestURI.isSipURI()) {
				...
				if (requestSipURI.getMAddrParam() != null) {
					...
					if (configuration.hasDomain(requestSipURI.getMAddrParam())) {
						...
						if (requestSipURI.getPort() != 5060
								&& requestSipURI.getPort() != -1) {
			...
			if (requestURI.isSipURI()) {
				...
				if (requestSipURI.getMAddrParam() != null) {
					...
					requestForwarding.forwardRequest(targetURIList,
							sipProvider, request, serverTransaction, true);
			...
			if (requestURI.isSipURI()) {
				...
				if (!configuration.hasDomain(requestSipURI.getHost())) {
					...
					requestForwarding.forwardRequest(targetURIList,
							sipProvider, request, serverTransaction, false);
			...
			if (request.getMethod().equals(Request.REGISTER)) {
				...
				registrar.processRegister(request, sipProvider,
						serverTransaction);
				// Henrik: let the presenceserver do some processing too
				if (isPresenceServer()) {
					presenceServer.processRegisterRequest(sipProvider, request,
							serverTransaction);
				}
			...
			if (isPresenceServer()
				...
				if (presenceServer.isStateAgent(request)) {
					...
					presenceServer.processSubscribeRequest(sipProvider,
							clonedRequest, serverTransaction);
				} else {
					// Do we know this guy?
					...
					if (targetURIList == null) {
						...
						ProxyDebug
								.println("Proxy: received a Subscribe request to "
						...
						ProxyDebug.println("Trying to forward subscribe to "
			...
			/**
			 * Received a Notify. TOADD: Check if it match active
			 * VirtualSubscriptions and update it
			 **/
			if (isPresenceServer()
				...
				if (serverTransaction != null)
					...
					sipProvider.sendResponse(response);
				ProxyDebug
						.println("Proxy: received a Notify request. Probably wrong, responded 481");
				return;
			}

			if (isPresenceServer()
					&& (request.getMethod().equalsIgnoreCase("PUBLISH"))) {
			...
			if (request.getMethod().equals(Request.BYE)) {
				...
				billingServer.callEnded(requestEvent);  //TODO foivos 25-3-2015
				
				Dialog d = serverTransaction.getDialog();
				TransactionsMapping transactionsMapping = (TransactionsMapping) d
						.getApplicationData();
				Dialog peerDialog = (Dialog) transactionsMapping
				...
				if (peerDialog.getState() != null) {
					ClientTransaction newct = sipProvider
					...
					if (ProxyDebug.debug)
						ProxyDebug
								.println("Proxy, bad dialog state - BYE dropped");
					return;
				}
			}

			// TODO foivos 28-3-2015
			if (request.getMethod().equals(Request.INVITE)) {
				billingServer.callCreated(requestEvent);
//				if (!forwardingServer.applyForwarding(requestEvent)) {
//					forwardingServer.sendDecline(requestEvent);
//					return;
//				}
//				else
//					System.out.println("#################\n" + request);
			...
			// TODO foivos 25-3-2015
			if (request.getMethod().equals(Request.ACK)) {
				billingServer.callStarted(requestEvent);
			}
			// call starts after receiving ACK from caller  
			// foivos end

			// Handle INFO messages (used for service requests)
			// TODO kostis 24-1-2015
			if (request.getMethod().equals(Request.INFO) &&
				request.getHeader(Constants.SERVICE) != null) {
			...
			if (requestURI.isSipURI()) {
				...
				if (ProxyDebug.debug)
					ProxyDebug
							.println("Proxy, processRequest(), we canonicalized"
									+ " the request-URI");
			...
			if (registrar.hasRegistration(request)
					&& !blockingServer.isBlocked(request)) {
				...
				if (targetURIList != null && targetURIList.size() > 1
					...
					if (ProxyDebug.debug)
						ProxyDebug
								.println("Proxy, processRequest(), the request "
				...
				if (targetURIList != null && !targetURIList.isEmpty()) {
					if (ProxyDebug.debug)
						ProxyDebug
								.println("Proxy, processRequest(), the target set"
			...
			ProxyDebug
					.println("Proxy, processRequest(), the registrar cannot help"
			...
			if (router != null) {
				...
				if (hop != null) {
					...
					SipURI hopURI = addressFactory.createSipURI(user,
							hop.getHost());
			...
			if (ProxyDebug.debug)
				ProxyDebug
						.println("Proxy, processRequest(), unable to set "
			...
			try {
				if (ProxyDebug.debug) {
					ProxyDebug
							.println("Proxy, processRequest(), internal error, "
									+ "exception raised:");
	 ...
	 * @throws Exception 
	 */
	private void handleServiceRequest(RequestEvent requestEvent)
	{
		Request request = requestEvent.getRequest();
		Header serviceHeader = request.getHeader(Constants.SERVICE);
		String headerStr = serviceHeader.toString().trim();
		String header = headerStr.replace(Constants.SERVICE + ": ","");

		if (header.equals(Constants.GET_BLOCKED_LIST)) {
			System.out.println("Got Blocking List Service Request");
			blockingServer.handleBlockedListRequest(requestEvent);
		} 
		else if (header.equals(Constants.BLOCK)) {
			System.out.println("Got Block Service Request");
			blockingServer.handleBlockRequest(requestEvent);
		} 
		else if (header.equals(Constants.UNBLOCK)) {
			System.out.println("Got Unblock Service Request");
			blockingServer.handleUnblockRequest(requestEvent);
		} 
		else if (header.equals(Constants.FORWARD)) {
			System.out.println("Got Forward Service Request");
			forwardingServer.handleForwardRequest(requestEvent);
		}
		else if (header.equals(Constants.UNFORWARD)) {
			System.out.println("Got Unforward Service Request");
			forwardingServer.handleUnforwardRequest(requestEvent);
		}
		else if (header.equals(Constants.GET_CURRENT_FORWARD)) {
			System.out.println("Got Current Forward Request");
			forwardingServer.handleForwardQuestionRequest(requestEvent);
		} 
		else if (header.equals(Constants.GET_BILL)) {
			System.out.println("Got Get Bill Service Request"); 
			billingServer.handleShowBillRequest(requestEvent);
		}
		else if (header.equals(Constants.CHARGE_BY_RATE) ||
				 header.equals(Constants.CHARGE_FIXED_PRICE) ||	
				 header.equals(Constants.FREE_OF_CHARGE)) {
			System.out.println("Got a Change Strategy Request");
			billingServer.handleChangeStrategyRequest(requestEvent);
		}
		else if (header.equals(Constants.CURRENT_STRATEGY)) {
			System.out.println("Got a Current Strategy Request");
			billingServer.handleCurrentStrategyRequest(requestEvent);
		}
		else if (header.equals(Constants.REGISTER)) {
			System.out.println("Got a Register Request");
			registerServer.handleRegisterRequest(requestEvent);
		}
		else if (header.equals(Constants.GET_FINAL_TARGET)) {
			System.out.println("Got a Final Target Request");
			forwardingServer.handleFinalTargetRequest(requestEvent);
		}
		else {
			ProxyDebug.println("Unknown sevice name: " + header);
		}
	}
	...
	public void processResponse(ResponseEvent responseEvent) {
		try {
			...
			if (cseqHeader.getMethod().equals("SUBSCRIBE")) {
				presenceServer.processSubscribeResponse(
						(Response) response.clone(), clientTransaction);
			} else if (cseqHeader.getMethod().equals("NOTIFY")) {
				// presenceServer.processNotifyResponse((Response)response.clone(),
				// clientTransaction);
			}

			responseForwarding.forwardResponse(sipProvider, response,
					clientTransaction);
	...
	public void processTimeout(TimeoutEvent timeOutEvent) {
		...
		if (timeOutEvent.isServerTransaction()) {
			...
			if (dialog != null) {
				transactionsMapping = (TransactionsMapping) dialog
						.getApplicationData();
			...
			if (dialog != null) {
				...
				if (st == null) {
					ProxyDebug
							.println("ERROR, Unable to retrieve the server transaction,"
									+ " cannot process timeout!");
					return;
				}
			} else {
				ProxyDebug
						.println("ERROR, Unable to retrieve the transaction Mapping,"
								+ " cannot process timeout!");
				return;
			}
	...
	public void start() throws Exception {
		if (configuration != null && configuration.isValidConfiguration()) {
			...
			ProxyDebug.println("The proxy is responsible for the domain:"
					+ configuration.stackIPAddress);
			...
			if (configuration.domainList != null)
				for (int j = 0; j < configuration.domainList.size(); j++) {
					String domain = (String) configuration.domainList
							.elementAt(j);
					ProxyDebug.println("Here is one domain to take care of:"
							+ domain);
				}
			else
				ProxyDebug.println("No domain to take care of...");
			...
			if (configuration.enableAuthentication) {
				...
				try {
					...
					Class authMethodClass = Class
							.forName(configuration.classFile);
					AuthenticationMethod authMethod = (AuthenticationMethod) authMethodClass
							.newInstance();
					...
					ProxyDebug
							.println("ERROR, authentication process stopped, exception raised:");
					e.printStackTrace();
				}
			}
			...
			for (int i = 0; lps != null && i < lps.size(); i++) {
				...
				try {
					...
					ListeningPoint lp = sipStack.createListeningPoint(Integer
							.valueOf(a.port).intValue(), a.transport);
			...
			if (configuration.enableRegistrations) {
				...
				ProxyDebug.println("Parsing the XML registrations file: "
						+ value);
				if (value == null || value.trim().equals(""))
					ProxyDebug
							.println("You have to set the registrations file...");
				else
					registrar.parseXMLregistrations(value);
			} else
				ProxyDebug.println("No registrations to parse...");
	...
	/**
	 * Exit the proxy, throws Exception that which can be caught by the upper
	 * application
	 */
	...
	public ViaHeader getStackViaHeader() {
		try {
			ListeningPoint lp = (ListeningPoint) sipStack.getListeningPoints()
					.next();
